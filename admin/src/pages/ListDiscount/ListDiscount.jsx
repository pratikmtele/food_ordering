...
const ListDiscount = () => {
  ...
  useEffect(() => {
    setSearchedItem(
      list.filter((item) => item.code.toLowerCase().includes(search))
    );
  }, [search]);

  const toggleActiveStatus = async (discountItem) => {
    const response = await axios.put(
      `${url}/api/discount/update/${discountItem._id}`,
      { ...discountItem, isActive: !discountItem.isActive }
    );
                ...
                <div key={index} className="discount-list-table-format">
                  ...
                  <div className="action-icons">
                    <Link to={`/discount/update/` + item._id}>
                      <img
                        className="cursor icons edit-icon"
                        src={assets.Edit_icon}
                      />
                    </Link>
                    <img
                      className="cursor icons"
